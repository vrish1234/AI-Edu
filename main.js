...
function showChatSection() {
  ...
  document.getElementById('loggedInName').innerText = user.name + " (" + user.email + ")";
  showUserHistory();
}
...
function clearChatUI() {
  ...
  document.getElementById('shortNotesText').innerText = '';
...
async function fetchAnswerFromGemini(question, language) {
  ...
  async function callAPI(text) {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "Answer not available.";
  }

  const answer = await callAPI(prompt);
  const shortNotes = await callAPI(`${prompt}\n${answer}\n${shortPrompt}`);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Setup theme toggle switch if exists
  const themeToggle = document.getElementById("themeToggle");
  if (themeToggle) {
